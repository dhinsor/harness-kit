---
name: spec-review
description: Review the changes since a fixed point (commit, branch, tag, or merge-base) along two axes — Standards (does the code follow this repo's documented coding standards?) and Spec (does the code match what the originating ticket/spec/PRD asked for?). Runs both reviews in parallel sub-agents and reports them side by side. Use when the user wants to check a branch or PR against its originating spec/ticket, distinct from /code-review's bug-hunt.
---
...
1. Issue/ticket references in the commit messages (`#123`, `Closes #45`, a Linear identifier, etc.) — fetch via Linear MCP or `gh` CLI as appropriate.
2. A path the user passed as an argument.
3. A spec/PRD file under `docs/`, `specs/`, `issues/`, or `.scratch/` matching the branch name or feature (this repo's `/to-spec` and `/to-tickets` write here).
4. If nothing is found, ask the user where the spec is. If they say there isn't one, the **Spec** sub-agent will skip and report "no spec available".

### 3. Identify the standards sources

Anything in the repo that documents how code should be written, such as `CODING_STANDARDS.md`, `CONTRIBUTING.md`, or `CONTEXT.md`.
